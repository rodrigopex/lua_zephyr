		...
		if (!(ttisstring(s2v(top - 2)) || cvt2str(s2v(top - 2))) ||
			...
			for (n = 1; n < total && tostring(L, s2v(top - n - 1)); n++) {
				...
				if (l_unlikely(l >= MAX_SIZE - sizeof(TString) - tl)) {
					L->top.p = top -
						   total; /* pop strings to avoid wasting stack */
      ...
      printf("line: %d\n", luaG_getfuncline(cl->p, pcRel(pc, cl->p)));
		...
		vmdispatch(GET_OPCODE(i))
		{
			...
			vmcase(OP_TAILCALL)
			{
				...
				if (TESTARG_k(i)) {
					luaF_closeupval(
						L, base); /* close upvalues from current call */
					lua_assert(L->tbclist.p <
						   base); /* no pending tbc variables */
					lua_assert(base == ci->func.p + 1);
				}
			...
			vmcase(OP_RETURN0)
			{
				if (l_unlikely(L->hookmask)) {
					...
					for (nres = ci->nresults; l_unlikely(nres > 0); nres--) {
						setnilvalue(
							s2v(L->top.p++)); /* all results are nil */
					}
				}
				goto ret;
			}
			vmcase(OP_RETURN1)
			{
				if (l_unlikely(L->hookmask)) {
					...
					if (nres == 0) {
						...
						StkId ra = RA(i);
						setobjs2s(L, base - 1,
							  ra); /* at least this result */
						L->top.p = base;
						for (; l_unlikely(nres > 1); nres--) {
							setnilvalue(s2v(
								L->top.p++)); /* complete missing
										 results */
						}
					}
				}
			...
			vmcase(OP_FORLOOP)
			{
				...
				if (ttisinteger(s2v(ra + 2))) { /* integer loop? */
					...
					if (count > 0) { /* still more iterations? */
						...
						lua_Integer idx =
							ivalue(s2v(ra)); /* internal index */
						chgivalue(s2v(ra + 1),
							  count - 1);      /* update counter */
						idx = intop(+, idx, step); /* add step to index */
						chgivalue(s2v(ra), idx); /* update internal index */
						setivalue(s2v(ra + 3),
							  idx);     /* and control variable */
	...
	vmbreak;
			...
			vmcase(OP_SETLIST)
			{
				...
				if (n == 0) {
					...
					L->top.p =
						ci->top.p; /* correct top in case of emergency GC */
				}
				last += n;
				if (TESTARG_k(i)) {
